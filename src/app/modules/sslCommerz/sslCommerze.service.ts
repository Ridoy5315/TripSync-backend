...
const sslPaymentInit = async (payload: ISSLCommerz): Promise<SSLCommerzInitResponse> => {
  try {
    const data = {
      ...
      success_url: `${envVars.SSLCOMMERZ.SSL_SUCCESS_BACKEND_URL}?id=${payload.rideId}&transactionId=${payload.transactionId}&amount=${payload.amount}&status=success`,
      fail_url: `${envVars.SSLCOMMERZ.SSL_FAIL_BACKEND_URL}?id=${payload.rideId}&transactionId=${payload.transactionId}&amount=${payload.amount}&status=FAIL`,
      cancel_url: `${envVars.SSLCOMMERZ.SSL_CANCEL_BACKEND_URL}?id=${payload.rideId}&transactionId=${payload.transactionId}&amount=${payload.amount}&status=CANCEL`,
